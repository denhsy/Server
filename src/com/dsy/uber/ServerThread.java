...
/**
 * All server code taken from Jon Berg
 */
...
extends Thread {
	...
	public ServerThread(int listen_port) {
	...
	private void http_handler(BufferedReader input, DataOutputStream output) {
		...
		try {
			...
			for (int a = 0; a < tmp2.length(); a++) {
				if (tmp2.charAt(a) == '?' ||  tmp2.charAt(a) == ' ' && start != 0) {
					end = a;
					break;
				}
			...
			//----------------------
			/**
			 * This section will parse javascript GET data and return data to the caller in raw string format
			 */
			StringTokenizer st;
			st = new StringTokenizer(tmp2);
			//Must have 3 tokens to continue
			if (st.countTokens() == 3){
				//have to ignore the first token
				st.nextToken();
				//deliminater is a ? 
				...
				if (st.countTokens() == 2){
					...
					st = new StringTokenizer(st.nextToken(), "&");
					if (st.countTokens() == 2){
						String x = st.nextToken(); //x value
						String y = st.nextToken(); //y value

						//only wish to parse the numeral section
						int xVal = Integer.parseInt(x.substring(2));
						int yVal = Integer.parseInt(y.substring(2));
						
						String returnString = "\n";
						ArrayList<FoodTruck> trucks = SFFoodTrucks.getInstance().getList(xVal, yVal);
						for (int i = 0; i < Server.NUM_OF_LISTINGS ; i++){
							returnString += i + ": " +trucks.get(i) + "\n";
						}
						//listings being sent to the web user
						output.writeChars(returnString);
						output.close();
						return;
